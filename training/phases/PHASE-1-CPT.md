...
> Имя файла сохранено для существующих cross-links. Broad CPT не является default run.

## Input, objective, output

- **[VERIFIED FACT] Input:** accepted Phase 0, immutable `U0` BF16 foundation/tokenizer, identity/tool contract examples, protected replay и lineage.
...
- **[VERIFIED FACT] Output:** retained `A1`, BF16 merged `M1=U0+A1`, fresh-optimizer record, merge parity и identity/capability reports.
...
- **[EXPERIMENT REQUIRED]** DAPT допускается только как отдельно approved ablation с отдельным profile, budget, data gate и no-regression criterion; он не создаёт default master lineage.
...
| Storage | **[ENGINEERING HYPOTHESIS]** Network Volume working set; encrypted registry stores `U0/A1/M1` and manifests. |
